...
def display_report(title: str, text: str):
    ...
    print(
        f"  • KL  (Unfamiliar Word %):      {s.unfamiliar_word_ratio:.2f}% ({s.unfamiliar_words} / {s.total_words})"
    )
    print(
        f"  • TTR (Type-Token Ratio %):     {s.type_token_ratio:.2f}% ({s.single_occurrence_words} unique stems)"
    )
    print(
        f"  • KCO (Complex Sentence %):     {s.complex_sentence_ratio:.2f}% ({s.complex_sentences} sentences)"
    )
    print(
        f"  • YCO (Fiilimsi / Clause Rate): {s.clause_ratio:.2f} ({s.total_fiilimsiler} fiilimsiler)"
    )

    print("\n--- 2. KALYONCU (2025) READABILITY SUITE ---")
    print(
        f"  [1] Formula 1 (Comprehensive, R²=0.99) : Score = {report.kalyoncu_formula1.score:>6.2f} -> {report.kalyoncu_formula1.grade_level}"
    )
    print(
        f"  [2] Formula 2 (Standard, R²=0.82)      : Score = {report.kalyoncu_formula2.score:>6.2f} -> {report.kalyoncu_formula2.grade_level}"
    )
    print(
        f"  [3] Formula 3 (Clause-based, R²=0.83)  : Score = {report.kalyoncu_formula3.score:>6.2f} -> {report.kalyoncu_formula3.grade_level}"
    )
    print(
        f"  [4] Formula 4 (Practical, R²=0.78)     : Score = {report.kalyoncu_formula4.score:>6.2f} -> {report.kalyoncu_formula4.grade_level}"
    )

    print("\n--- 3. COMPARATIVE CLASSIC FORMULAS ---")
    print(
        f"  • Ateşman (1997)                       : Score = {report.atesman.score:>6.2f} -> {report.atesman.grade_level}"
    )
    print(
        f"  • Çetinkaya-Uzun (2010)                : Score = {report.cetinkaya_uzun.score:>6.2f} -> {report.cetinkaya_uzun.grade_level}"
    )
    print(
        f"  • Bezirci-Yılmaz (2010)                : Score = {report.bezirci_yilmaz.score:>6.2f} -> {report.bezirci_yilmaz.grade_level}"
    )
    print("\n")
...
def main():
    ...
    display_report(
        "Sample 1: Primary School Story (İki Horoz - 3 ve 4. Sınıf)", iki_horoz_story
    )
    display_report(
        "Sample 2: Academic Paper (Lisans & Lisansüstü Düzeyi)", academic_paper
    )
