...
def main():
    ...
    print(
        f"    • Total sentences: {report1.metrics['rhythm']['total_sentences']}, Total words: {report1.metrics['rhythm']['total_words']}"
    )
    print(
        f"    • Mean sentence length: {report1.metrics['rhythm']['mean_sentence_length']:.1f} words (Std Dev: {report1.metrics['rhythm']['std_dev_sentence_length']:.1f})"
    )
    print(
        f"    • '-mektedir/-maktadır' predicates: {report1.metrics['predicates']['mektedir_count']} (%{report1.metrics['predicates']['mektedir_ratio'] * 100:.0f})"
    )
    print(
        f"    • Punctuation anomalies: {report1.metrics['punctuation']['em_dash_count']} em-dashes (—), {report1.metrics['punctuation']['semicolon_count']} semicolons (;)"
    )
    print(
        f"    • Clichés & Calques: {report1.metrics['bureaucratic_connectors_count']} bureaucratic, {report1.metrics['conclusion_cliches_count']} conclusion hedges, {report1.metrics['rhetorical_calques_count']} rhetorical calques"
    )
    ...
    print("\n" + "-" * 80)
    print(
        " GENERATED ACTIONABLE HUMANIZER PROMPT (for Claude / ChatGPT / Human Editor):"
    )
    print("-" * 80)
    ...
    print(
        f"[*] Predicates: {report2.metrics['predicates']['mektedir_count']} '-mektedir' endings"
    )
    print(
        f"[*] Rhythm: Short sentences count = {report2.metrics['rhythm']['short_sentences_count']}, Mean = {report2.metrics['rhythm']['mean_sentence_length']:.1f} words"
    )
