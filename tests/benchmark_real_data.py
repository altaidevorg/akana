...
def evaluate_split(pylist, split_name="In-Domain Test Set", limit=None):
    ...
    print("\n" + "=" * 80)
    print(
        f" EVALUATING AKANA ON REAL DATASET: {split_name} ({len(pylist):,} sentences)"
    )
    print("=" * 80)
    ...
    for row_idx, row in enumerate(pylist):
        ...
        if (row_idx + 1) % 5000 == 0 or (row_idx + 1) == len(pylist):
            ...
            print(
                f"  Processed {row_idx + 1:,}/{len(pylist):,} sentences ({cur_speed:,.1f} sent/s)..."
            )
    ...
    print(
        f"  • Throughput:                {throughput:,.1f} sentences/sec ({total_tokens / elapsed:,.1f} tokens/sec)"
    )
    print(
        f"  • Avg Latency:               {latency_us:.2f} µs/sentence ({latency_us / 1000:.4f} ms)"
    )
    ...
    print(f"  • F1 Score:                  {f1_sent * 100:.2f}%")
...
if __name__ == "__main__":
    ...
    movie_reviews_data = download_dataset(
        "data/movie_reviews-00000-of-00001-b0322cba85108ee9.parquet"
    )
    movie_results = evaluate_split(
        movie_reviews_data, split_name="Human Movie Reviews (Out-of-Domain Test Set)"
    )
    ...
    test_results = evaluate_split(
        test_data, split_name="Full GECTurk In-Domain Test Set (20,769 Sentences)"
    )
    ...
    print("=" * 80)
    print(
        f"{'Benchmark Dataset':<35} {'Throughput':<16} {'Latency':<12} {'Precision':<10} {'Recall':<10} {'F0.5':<10}"
    )
    print("-" * 95)
    print(
        f"{'Human Movie Reviews (300 sents)':<35} {movie_results['throughput']:,.0f} sent/s   {movie_results['latency_ms']:.3f} ms     {movie_results['p_tok'] * 100:.1f}%      {movie_results['r_tok'] * 100:.1f}%     {movie_results['f05_tok'] * 100:.1f}%"
    )
    print(
        f"{'Full In-Domain Test (20,769 sents)':<35} {test_results['throughput']:,.0f} sent/s   {test_results['latency_ms']:.3f} ms     {test_results['p_tok'] * 100:.1f}%      {test_results['r_tok'] * 100:.1f}%     {test_results['f05_tok'] * 100:.1f}%"
    )
