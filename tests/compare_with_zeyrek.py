...
def main():
    ...
    print(
        f"  -> Akana is {t_zeyrek_init / max(t_akana_init, 1e-6):.1f}x faster to initialize!"
    )
    ...
    print(
        f"  - Akana Throughput: {akana_wps:,.0f} words/sec ({akana_elapsed * 1000:.1f} ms total)"
    )
    print(
        f"  - Zeyrek Throughput: {zeyrek_wps:,.0f} words/sec ({zeyrek_elapsed * 1000:.1f} ms total)"
    )
